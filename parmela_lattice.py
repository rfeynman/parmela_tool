...
file_name = "OUTPAR_sep.TXT"
full_file_path = os.path.join(file_path, file_name)

# Output filenames
output_excel = os.path.join(file_path, f"lattice_{file_name.replace('.TXT', '')}.xlsx")
output_plot = os.path.join(file_path, f"layout_{file_name.replace('.TXT', '')}.png")
...
def generate_lattice_plot(df, save_path):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(36, 10))
    plt.subplots_adjust(hspace=0.4)
    
    # Common Style Settings
    lw_box = 2.5      # Line width for boxes
    lw_drift = 2.0    # Line width for drift
    ...
    for idx, row in df.iterrows():
        ...
        if 'drift' in name:
            ...
            # Darker cyan/blue for better visibility
    ...
    ax1.set_xlabel("S [m]", fontsize=font_size)
    ...
    # ===========================
    # Plot 2: Top View Layout (Bottom Plot)
    # ===========================
    legend_patches2 = {}
    
    total_bend_deg = df[df['element'].str.lower().str.contains('bend')]['angle'].sum()
    ...
    for idx, row in df.iterrows():
        ...
        if 'bend' in name:
            ...
            if abs(bend_rad) > 1e-9:
                ...
                # Simple Polygon for visibility (since a rotated rectangle doesn't bend)
                width = 0.15
...
if __name__ == "__main__":
    ...
    req_data, lattice_data = process_parmela_file(full_file_path)
    ...
    if req_data and lattice_data:
        ...
        generate_lattice_plot(df_lattice, output_plot)
        print("Done.")
    else:
